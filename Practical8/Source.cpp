...
LRESULT WINAPI WindowProcedure(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KEYDOWN:
		...
		case 'X':
			lightPos0[1] += light1MoveSpeed;
			break;
		case 'C':
			lightPos0[0] -= light1MoveSpeed;
			break;
		case 'V':
			lightPos0[1] -= light1MoveSpeed;
			break;
...
void setupLight()
{
	if (isLight) {
		...
		// Enable and configure the second light source (GL_LIGHT1)
		glEnable(GL_LIGHT1); // Second light source

		// Define the position and properties of the second light
		GLfloat lightPos2[] = { 2.0f, 2.0f, 2.0f, 1.0f }; // Change this as needed for light position
		...
		// Draw a small sphere to represent the second light position
		glPushMatrix();
		glDisable(GL_LIGHTING);  // Disable lighting for light source visualization
		glTranslatef(lightPos0[0], lightPos0[1], lightPos0[2]);
		glColor3f(0, 0, 1);  // Blue color for the second light source
		GLUquadric* lightSphere2 = gluNewQuadric();
		gluSphere(lightSphere2, 0.05, 50, 50);
		gluDeleteQuadric(lightSphere2);
		glEnable(GL_LIGHTING);
		glPopMatrix();
	}
	else {
...
void display()
{
	...
	GLfloat matrix_amb2[16] = { lightPos0[1],  0,  0, 0,
							-lightPos0[0], 0,  -lightPos0[2], -1,
							0,  0,  lightPos0[1],0,
							0,  0,  0,lightPos0[1],
	};
	...
	case 1:
		...
		if (isSphere) {
			...
			glPushMatrix();
			glTranslatef(0.0f, -0.99f, 0.0f);  // Move down by 0.9f on the Y-axis
			glDisable(GL_DEPTH_TEST);
			glDisable(GL_LIGHTING);
			glMultMatrixf(matrix_amb);         // Apply custom matrix for lighting or other effects
			drawSphereShadow();
			glEnable(GL_LIGHTING);
			...
			glDisable(GL_LIGHTING);
			glColor3f(1, 0, 0);
			glMultMatrixf(matrix_amb2);         // Apply custom matrix for lighting or other effects
			drawSphereShadow2();
			glEnable(GL_LIGHTING);
			glEnable(GL_DEPTH_TEST);
			glPopMatrix();
