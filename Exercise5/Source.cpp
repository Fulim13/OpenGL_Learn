...
float prx = 0;                       //rotate rx for projection
//cloud controller
...
LRESULT WINAPI WindowProcedure(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KEYDOWN:
		...
		else if (wParam == 'A') {            //Rotate Projection at x axis (anti-clockwise)
			prx += prSpeed;
		}
		else if (wParam == 'D') {            //Rotate Projection at x axis (clockwise)
			prx -= prSpeed;
		}
		...
		// ptx
		else if (wParam == 'U') {            //Translate Projection to up hand side
					ptx += tSpeed;
				}
		else if (wParam == 'J') {            //Translate Projection to down hand side
					ptx -= tSpeed;
				}

		// pty
		else if (wParam == 'I') {            //Translate Projection to up hand side
					pty += tSpeed;
				}
		else if (wParam == 'K') {            //Translate Projection to down hand side
					pty -= tSpeed;
				}
		else if (wParam == 'C') {
			cloudMain = true;
